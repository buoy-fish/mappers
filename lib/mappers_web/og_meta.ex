defmodule MappersWeb.OgMeta do
  ...
  the exact framed area via the Mapbox Static Images API. Anything missing or
  invalid degrades to the branded card.

  Pure and free of Plug/Conn — the controller supplies `base_url`, `page_url`,
  and `mapbox_token` so this stays trivially unit-testable.
  """
  ...
  def build(params, opts) do
    ...
    case params["play"] do
      play when is_binary(play) and play != "" ->
        %{
          title: humanize(play) <> " coverage timeline · Buoy.Fish",
